...
<script type="text/javascript">
	function userDelete(id){
		if(confirm("确定要删除这条记录吗?")){
			$.post("${pageContext.request.contextPath}/dataDic/delete.do",{id:id},
				function(result){
	...
	<table class="table table-hover  table-bordered table-striped" style="margin-top: 10px;">
	  ...
	  <c:forEach var="dataDic" items="${dataDicList }" varStatus="status">
	  	<tr>
	  		...
	  		<td>
	  			<button type="button" class="btn btn-info btn-xs" onclick="javascript:window.location.href='${pageContext.request.contextPath}/dataDic/preSave.do?id=${dataDic.ddId }'">修改</button>
